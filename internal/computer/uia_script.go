// Package computer provides desktop automation.
package computer

// windowsUIAScript returns the Windows UIA PowerShell helper script ported from
// openhanako's windows-uia-script.js. It supports commands: status, list_apps,
// get_app_state, perform_action (click_element, double_click, type_text,
// scroll, click_point, press_key, drag, stop).
func windowsUIAScript() string {
	return `
param(
  [string]$RequestPath = "",
  [string]$ResultPath = ""
...
function Write-Result($obj) {
  ...
  if (-not [string]::IsNullOrWhiteSpace($script:ResultPath)) {
    $resultDirectory = [System.IO.Path]::GetDirectoryName($script:ResultPath)
    if ([string]::IsNullOrWhiteSpace($resultDirectory)) {
      $resultDirectory = [System.IO.Directory]::GetCurrentDirectory()
    }
    $resultFileName = [System.IO.Path]::GetFileName($script:ResultPath)
    $tempResultPath = [System.IO.Path]::Combine(
      $resultDirectory,
      "$resultFileName.tmp.$PID.$([System.Guid]::NewGuid().ToString('N'))"
    )
    try {
      [System.IO.File]::WriteAllText($tempResultPath, $json, $script:Utf8NoBom)
      [System.IO.File]::Move($tempResultPath, $script:ResultPath)
    } catch {
      try {
...
function Test-Element-Matches-Snapshot($el, $snapshot) {
  ...
  $role = ""; $label = ""; $automationId = ""; $nativeWindowHandle = 0
  ...
  if ($snapshot.automationId -and ([string]$snapshot.automationId) -ne $automationId) {
    return @{ ok = $false; reason = "automation-id-mismatch" }
  }
  if ($snapshot.label -and ([string]$snapshot.label) -ne $label) {
    return @{ ok = $false; reason = "label-mismatch" }
  }
...
function Element-Center($el) {
  $r = $el.Current.BoundingRectangle
  return @{ x = [int]([Math]::Round($r.Left + ($r.Width / 2))); y = [int]([Math]::Round($r.Top + ($r.Height / 2))) }
}
...
try {
  ...
  if ($req.command -eq "get_app_state") {
    ...
    Write-Result @{ ok = $true; data = @{
      ...
      screenshot = $capture
      display = @{ x = 0; y = 0; width = $capture.width; height = $capture.height; scaleFactor = $capture.scaleFactor }
      elements = @($elements)
    } }
    exit 0
  }
  ...
  if ($req.command -eq "perform_action") {
    ...
    if ($null -eq $el -and $req.action.elementId) {
    ...
    if ($req.action.type -eq "click_element") {
      try {
        ...
        Write-Result @{ ok = $true; data = @{ ok = $true; mode = "foreground"; x = $pt.x; y = $pt.y } }
      }
      exit 0
    }
    ...
    if ($req.action.type -eq "type_text") {
      try {
        ...
        Write-Result @{ ok = $true; data = @{ ok = $true; mode = "foreground"; textLength = ([string]$req.action.text).Length } }
      }
      exit 0
    }
    if ($req.action.type -eq "scroll") {
      try {
        ...
        Write-Result @{ ok = $true; data = @{ ok = $true; mode = "foreground"; direction = $req.action.direction } }
      }
      exit 0
    }
  }
