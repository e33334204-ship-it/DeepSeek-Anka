...
func TestReadFileRef(t *testing.T) {
	...
	if got, isDir, err := readFileRef(context.Background(), textPath, "", ""); err != nil || isDir || got != "line one\nline two\n" {
		t.Errorf("text file = (%q, %v, %v)", got, isDir, err)
	}

	// Binary file: noted, not dumped.
	if got, _, err := readFileRef(context.Background(), binPath, "", ""); err != nil || !strings.Contains(got, "binary file") {
		t.Errorf("binary file = (%q, %v), want a binary note", got, err)
	}

	// Image file: identified as image-specific guidance, not generic binary.
	if got, _, err := readFileRef(context.Background(), imagePath, "", ""); err != nil || !strings.Contains(got, "image file") {
		t.Errorf("image file = (%q, %v), want an image note", got, err)
	}

	// Large file: truncated with a marker.
	if got, _, err := readFileRef(context.Background(), bigPath, "", ""); err != nil || !strings.Contains(got, "truncated") {
	...
	got, isDir, err := readFileRef(context.Background(), dir, "", "")
	if err != nil || !isDir {
	...
	// Missing path: error.
	if _, _, err := readFileRef(context.Background(), filepath.Join(dir, "nope"), "", ""); err == nil {
