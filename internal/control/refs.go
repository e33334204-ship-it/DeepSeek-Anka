...
func (c *Controller) ResolveRefs(ctx context.Context, line string) (block string, errs []string) {
	...
	for _, r := range refs {
		...
		case refFile:
			text, isDir, err := readFileRef(ctx, r.path, line, c.sessionPath)
			if err != nil {
				errs = append(errs, "@"+r.raw+" — "+err.Error())
				continue
			}
		...
		case refImage:
			body := "[image attachment at @" + r.path + "]"
			if br := capabilities.VisionBridge(); br != nil && capabilities.VisionEnabled() {
				path := r.path
				if clean, err := cleanAttachmentPath(path); err == nil {
					path = clean
				}
...
// When vision auxiliary is enabled, image files use cached session notes when
// prepareVisionForInput has already analyzed them.
func readFileRef(ctx context.Context, path, userLine, sessionPath string) (content string, isDir bool, err error) {
	...
	if mime := imageMime(data, path); mime != "" {
		body := fmt.Sprintf("[image file %s, mime=%s, %d bytes]", path, mime, info.Size())
		if br := capabilities.VisionBridge(); br != nil && capabilities.VisionEnabled() {
			if entry := br.LookupNote(sessionPath, path); entry != nil && strings.TrimSpace(entry.Note) != "" {
				body = vision.WrapNote(entry.Note) + "\n\n" + body
			}
		} else {
			body += " — configure [vision] with a separate vision-capable model for text-only chat models"
		}
