...
const attachmentRootDir = ".deepseek-anka/attachments"
...
// CleanAttachmentPath validates and resolves a repo-relative attachment path.
func CleanAttachmentPath(path string) (string, error) {
	return cleanAttachmentPath(path)
}
...
func attachmentPath(ext string) string {
	...
	return filepath.Join(filepath.FromSlash(attachmentRootDir), name)
}
