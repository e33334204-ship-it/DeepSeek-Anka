...
type Agent struct {
	...
	// PreStreamMutator, when non-nil, transforms the message list just before
	// each LLM stream request. The agent copies the session messages and passes
	// the copy to the mutator; the returned messages are what the provider sees.
	// The session's stored messages are never mutated. Used by the vision
	// pipeline to inject visual context notes into user messages.
	PreStreamMutator func(ctx context.Context, msgs []provider.Message) ([]provider.Message, error)
...
// ProjectChecks are host-observable structured checks extracted during boot.
	...
	// each LLM stream request. The agent copies the session messages and passes
	// the copy to the mutator; the returned messages are what the provider sees.
	// The session's stored messages are never mutated.
	PreStreamMutator func(ctx context.Context, msgs []provider.Message) ([]provider.Message, error)
}
...
func New(prov provider.Provider, tools *tool.Registry, session *Session, opts Options, sink event.Sink) *Agent {
	...
	return &Agent{
		...
		PreStreamMutator:  opts.PreStreamMutator,
	}
}
...
func (a *Agent) stream(ctx context.Context, turn int) (string, string, string, []provider.ToolCall, *provider.Usage, error) {
	...
	if a.PreStreamMutator != nil {
		var err error
		msgs, err = a.PreStreamMutator(ctx, append([]provider.Message(nil), msgs...))
		if err != nil {
			return "", "", "", nil, nil, fmt.Errorf("pre-stream mutator: %w", err)
		}
	}
